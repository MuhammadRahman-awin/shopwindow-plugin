...
function datafeed_init() {
	$plugin = new Datafeed_Plugin(); // Create container
	...
	$plugin['settings_page'] = 'datafeed_service_settings';
	$plugin->run();
}

function datafeed_service_settings( $plugin )
{
	$object = new Datafeed_SettingsPage( $plugin['settings_page_properties'] );
	return $object;
}
